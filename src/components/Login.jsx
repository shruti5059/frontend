...
export const Login = ({ setIsAuthenticated }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Store tokens and username and username in localStorage
            localStorage.setItem("accessToken", response.data.accessToken);
            localStorage.setItem("refreshToken", response.data.refreshToken);
            localStorage.setItem("username", formData.username);
            localStorage.setItem("userId", response.data.userId); // Ensure this is stored
    ...
    return (
        <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900">
            <div className="w-full max-w-md bg-gray-900 p-8 rounded-2xl shadow-xl">
                <h2 className="text-4xl font-extrabold text-center text-blue-400 mb-6">Welcome Back</h2>
                ...
                <p className="mt-6 text-center text-gray-400">
                    ...
                    <Link to="/register" className="text-blue-400 hover:underline ml-1">Register</Link>
                </p>
            </div>
        </div>
    );
